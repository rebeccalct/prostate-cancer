...
def umap_transform(data_scaled, labels):
    """
    Compute the UMAP transform from scaled data.
    ...
    umap_transform = umap.UMAP(n_components=2).fit_transform(data_scaled)

    # Append labels to the transformed data
    umap_data = np.append(umap_transform, labels.reshape(-1, 1), axis=1)

    # Return a dataframe with the transformed data
    return pd.DataFrame(data=umap_data, columns=['Component 1', 'Component 2', 'Label'])
...
def umap_transform_on_batch(data_scaled, labels):
    ...
    # Initialize dataframe to save the values of the transformation
    umap_df = pd.DataFrame(columns=['Component 1', 'Component 2', 'Label'])
    
    # Function to generate batches - assuming it's defined or available elsewhere
    def batch(iterable, n=1):
        l = len(iterable)
        for ndx in range(0, l, n):
            yield iterable[ndx:min(ndx + n, l)]
    ...
    for (b, y) in zip(batch(data_scaled, 10000), batch(labels, 10000)):
        ...
        # Transform the batch of data
        umap_transform = umap.UMAP(n_components=2).fit_transform(b)

        # Append the labels of the data
        umap_batch_data = np.append(umap_transform, y.reshape(-1, 1), axis=1)

        # Append the transformation of the actual batch to the dataframe
        umap_batch_df = pd.DataFrame(data=umap_batch_data, columns=['Component 1', 'Component 2', 'Label'])
        umap_df = umap_df.append(umap_batch_df, ignore_index=True)
